...
import { usePreqinDeals, usePreqinStats, formatAUM, formatNumber, formatDate } from '@/services/preqinApi';
import type { DealsParams } from '@/types/preqin';

export function PreqinDealsPage() {
  const [params, setParams] = useState<DealsParams>({ page: 1, page_size: 20 });
  const [searchInput, setSearchInput] = useState('');

  const { data, isLoading } = usePreqinDeals(params);
  ...
  const handleFilterChange = (key: keyof DealsParams, value: string) => {
          ...
          {stats && (
            <div className="flex items-center space-x-6 text-sm">
              <div className="text-center">
                <div className="text-lg font-bold text-white">{formatNumber(stats.total_deals)}</div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-800">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Target Company</th>
                  <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Deal Type</th>
                  <th className="px-4 py-3 text-center text-xs font-medium text-slate-400 uppercase tracking-wider">Date</th>
                  <th className="px-4 py-3 text-right text-xs font-medium text-slate-400 uppercase tracking-wider">Deal Value</th>
                ...
                {isLoading ? (
                  ...
                  data.items.map((deal) => (
                    <tr key={deal.id} className="hover:bg-slate-700/30 transition-colors">
                      <td className="px-4 py-3 text-sm text-white font-medium">{deal.target_company_name || '-'}</td>
                      <td className="px-4 py-3 text-sm">
                        <span className="px-2 py-1 rounded text-xs font-medium bg-amber-500/20 text-amber-400">
                          {deal.deal_type || '-'}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-slate-300 text-center">{formatDate(deal.deal_date)}</td>
                      <td className="px-4 py-3 text-sm text-slate-300 text-right">{formatAUM(deal.deal_value_usd)}</td>
                      <td className="px-4 py-3 text-sm text-slate-300">{deal.primary_industry || '-'}</td>
                      <td className="px-4 py-3 text-sm text-slate-300">{deal.country || '-'}</td>
                      <td className="px-4 py-3 text-sm text-slate-300">{deal.stage || '-'}</td>
                      <td className="px-4 py-3 text-sm">
                        <span className={`px-2 py-1 rounded text-xs font-medium ${
                          deal.deal_status === 'Completed' ? 'bg-emerald-500/20 text-emerald-400' :
                          deal.deal_status === 'Pending' ? 'bg-blue-500/20 text-blue-400' :
