/**
 * Deals Export Page
 * Displays private market deals from Preqin Excel data
 */
...
export function CleanDataDealsPage() {
  const [params, setParams] = useState<SheetDataParams>({ page: 1, page_size: 50 });
  const [searchInput, setSearchInput] = useState('');

  const { data: dataset, isLoading: datasetLoading } = useDataset('deals-dataset');
  const { data: sheetData, isLoading: sheetLoading } = useSheetData('deals-dataset', 'deals', params);
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      {/* Stats bar */}
      {dataset && (
        <div className="px-6 py-3 border-b border-slate-700/20 bg-slate-800/30">
          <div className="flex items-center space-x-6 text-sm">
            <div className="text-slate-400">
              Total Deals: <span className="text-white font-medium">{dataset.sheets[0]?.row_count.toLocaleString()}</span>
            </div>
            <div className="text-slate-400">
              Columns: <span className="text-white font-medium">{dataset.sheets[0]?.column_count}</span>
            </div>
          </div>
        </div>
      )}
