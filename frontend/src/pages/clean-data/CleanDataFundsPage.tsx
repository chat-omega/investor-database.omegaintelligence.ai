/**
 * Private Market Funds Page
 * Displays fund information and contacts from Preqin Excel data
 */
...
export function CleanDataFundsPage() {
  const [activeSheet, setActiveSheet] = useState('funds');
  const [params, setParams] = useState<SheetDataParams>({ page: 1, page_size: 50 });
  const [searchInput, setSearchInput] = useState('');

  const { data: dataset, isLoading: datasetLoading } = useDataset('funds-dataset');
  const { data: sheetData, isLoading: sheetLoading } = useSheetData('funds-dataset', activeSheet, params);
      ...
      <div className="border-b border-slate-700/20 bg-slate-900 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h1 className="text-xl font-bold text-white">Private Market Funds</h1>
              <p className="text-sm text-slate-400">Fund information and performance from Preqin</p>
            </div>
          </div>
